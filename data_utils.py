...
def generate_custom_data(n_samples=500, data_type='linear', noise_prob=0.05):
    # Кастомный генератор данных для бонусного задания
    np.random.seed(42)

    if data_type == 'linear':
        # Два линейно разделимых гауссовых облака
        ...
        # Нелинейные данные (проблема XOR)
        X = np.random.randn(n_samples, 2)
        y = np.logical_xor(X[:, 0] > 0, X[:, 1] > 0).astype(int)

    elif data_type == 'circle':
        # Нелинейные данные (концентрические области)
        X = np.random.randn(n_samples, 2) * 2
        radius = np.sqrt(X[:, 0] ** 2 + X[:, 1] ** 2)
        y = (radius > 2.0).astype(int)

    # Добавление контролируемого уровня шума
    if noise_prob > 0:
...
def standardize(X_train, X_test):
    # Z-нормализация (стандартизация признаков)
...
def stratified_split(X, y, test_size=0.3):
    # Разделение выборки с сохранением пропорций классов
    classes = np.unique(y)
    train_indices = []
    test_indices = []
    ...
    # Перемешивание итоговых индексов
